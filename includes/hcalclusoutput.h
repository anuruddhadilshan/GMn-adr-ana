...
#ifndef HCALCLUSOUTPUT_H
#define HCALCLUSOUTPUT_H

class HCalClusOutput
{
private:
	...
	double m_bbtrgemnhits {0.};
	double m_bbtrchi2divndof {0.};
	double m_bbtrvz{0.};
	double m_bbtrth{0.};
	double m_bbtrx{0.};
	double m_bbtrp{0.};
...
public:
	...
	HCalClusOutput(Event& event, const char* outputfilename) : m_event{event}, m_outputfilename{outputfilename}
	{
		m_fout = new TFile(Form("%s.root",outputfilename),"RECREATE");
		m_resultstree = new TTree("T","Down beding track analysis");

		m_resultstree->Branch("bb.tr.gemnhits", &m_bbtrgemnhits);
		m_resultstree->Branch("bb.tr.bbtrchi2divndof", &m_bbtrchi2divndof);
	...
	void copyFromEvent()
	{
		m_bbtrgemnhits = m_event.return_BBTrGEMNhits();
		m_bbtrchi2divndof = m_event.return_BBTrChi2divNdof();
