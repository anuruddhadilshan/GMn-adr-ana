...
#include "../includes/calc_q_W2.h"

const double target_mass{0.5*(0.938272+0.939565)}; //Average of neutron and proton mass.
constexpr int n_histos{13}; // The number of histograms that needs to be printed into the PDF.
...
void definecuts(const int kine_num, const char* configfilename = "setup_definecuts.cfg", TString outputfilename  = "setup_analysis")
{
	...
	double sh_clusblk_ADC_time[15];
	double hcal_atimeblk{0.};
	double sh_atimeblk{0.};
	...
	C->SetBranchAddress("bb.sh.clus_blk.atime",sh_clusblk_ADC_time);
	C->SetBranchAddress("sbs.hcal.atimeblk",&hcal_atimeblk);
	...
	TH1D* h1_hcal_clusblk_ADCtime = new TH1D("h1_hcal_clusblk_ADCtime","HCal ADC time of the highest energy block in the largest cluster; ADC Time (ns)",300,-100,200);
	TH1D* h1_hcal_atimeblk = new TH1D("h1_hcal_atimeblk","HCal ADC time of the highest energy block in the largest cluster; ADC Time (ns)",300,-100,200);
	TH1D* h1_sh_clusblk_ADCtime = new TH1D("h1_sh_clusblk_ADCtime","SH ADC time of the highest energy block in the largest cluster; ADC Time (ns)",200,-100,100);
	TH1D* h1_sh_atimeblk = new TH1D("h1_sh_atimeblk","SH ADC time of the highest energy block in the largest cluster; ADC Time (ns)",200,-100,100);
	TH1D* h1_hcal_sh_atimeblkdiff = new TH1D("h1_hcal_sh_atimeblkdiff","HCal ADC time - SH ADC time; HCalADC_{time}-SHADC_{time} (ns); Entries",300,-100,200);
	...
	while (C->GetEntry(nevent++))
	{
		...
		h1_hcal_atimeblk->Fill(hcal_atimeblk);

		h1_sh_clusblk_ADCtime->Fill(sh_clusblk_ADC_time[0]);

		h1_sh_atimeblk->Fill(sh_atimeblk);

		double hcal_sh_atimeblkdiff{ hcal_atimeblk - sh_atimeblk };

		h1_hcal_sh_atimeblkdiff->Fill(hcal_sh_atimeblkdiff);
	...
	TString coincut_pngName = outputfilename+"_coincut.png";
 	...
 	c[9]->cd();
 	h1_hcal_atimeblk->Draw();

 	c[10]->cd();
 	h1_sh_clusblk_ADCtime->Draw();

 	c[11]->cd();
 	h1_sh_atimeblk->Draw();

 	c[12]->cd();
 	h1_hcal_sh_atimeblkdiff->Draw();
