...
void calc_pionprod_thres_withpolangle(double E_beam = 4.0268, double theta_deg = 49.0)
{
	...
	double E_gamma_min = ( 2*p_mass*piplus_energy_limit + std::pow(n_mass,2) - std::pow(piplus_mass,2) - std::pow(p_mass,2) ) / ( 2*( p_mass + piplus_momentum_limit*cos(TMath::DegToRad()*theta_deg) - piplus_energy_limit ) ); 
