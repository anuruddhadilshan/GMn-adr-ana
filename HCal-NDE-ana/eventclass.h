...
private:
	...
	bool hit_In_Circle()
	{
		double r = std::sqrt( std::pow(m_sbsneutronhcaldx,2) + std::pow(m_sbsneutronhcaldy,2) );
...
public:	
	...
	void eval_HCalNDE_Rmthd()
	{
		...
		m_pass_hcalshcoincut = pass_HCalSHADCtime_coincut(); //	Reqire for the HCal and BB SH ADC time to be in a tight correlation.

		if ( m_hit_incircle && m_pass_hcalshcoincut )
		{
			m_nhcal_didhit++;
			m_pass_rmthd = true;
		}	
