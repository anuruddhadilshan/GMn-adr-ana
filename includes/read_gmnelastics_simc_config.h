...
#ifndef READ_GMNELASTICS_SIMC_CONFIG_H
#define READ_GMNELASTICS_SIMC_CONFIG_H

class ConfigfileSIMC 
{
private:
	//Defining the variables read in by the configuration file.
	std::vector<TString> m_ROOTFileVector;
	TString m_CSVFile {""};
	int m_processint {0};
	TString m_process_string {""};
	int m_SBSKineNum {0}; 
	TString m_Target {""};
...
public:
	...
	ConfigfileSIMC() = default;	

	ConfigfileSIMC( const char* configfilename )
	{
	  		...
	  		if ( !currentline.BeginsWith("#") )
	  		{
		    	...
		    	if( ntokens > 1 )
		    	{
		    		...
		    		if ( skey == "CSVFile" )
		    		{
		    			...
		    			m_CSVFile = sval;
		    			std::cout << "CSV File from JLab-HPC: " << m_CSVFile << '\n';
		    		}

		    		if ( skey == "Process" )
		    		{
		    			TString sval = ( (TObjString*)(*tokens)[1] )->GetString();
		    			m_processint = sval.Atoi();

		    			switch (m_processint)
		    			{
		    				case 0:
		    					m_process_string = "deep";
		    					break;
		    				case 1:
		    					m_process_string = "deen";
		    					break;
		    				case 2:
		    					m_process_string = "heep";
		    					break;
		    				default:
		    					m_process_string = "xprocess";
		    					break;
		    			}

		    			std::cout << "SIMC event generation process: " << m_process_string.Data() << '\n';
		    		}
	...
	std::vector<TString> return_ROOTFileVector()
	{
		return m_ROOTFileVector;
	}

	TString return_CSVFile()
	{
		return m_CSVFile;
	}

	TString return_SIMCprocess()
	{
		return m_process_string;
	}
